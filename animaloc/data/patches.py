...
class ImageToPatches:
    ...
    def make_patches(self) -> torch.Tensor:
        ...
        if residual != 0:
            ...
            remaining_height = torch.zeros(n_channels, 1, self.image.size(2), height,
                                          dtype=self.image.dtype, device=self.image.device) # padding
        ...
        if residual != 0:
            remaining_width = torch.zeros(n_channels, fold.shape[1], 1, height, width,
                                         dtype=self.image.dtype, device=self.image.device) # padding
